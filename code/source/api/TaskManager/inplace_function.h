// Taken from https://raw.githubusercontent.com/WG21-SG14/SG14/master/SG14/inplace_function.h
...
constexpr size_t InplaceFunctionDefaultCapacity = 64;
...
private:
	...
	[[noreturn]] static RetT DefaultFunction( ArgsT..., const void* )
	{
#ifdef _MSC_VER
		__debugbreak();
#else //#ifdef _MSC_VER
	...
	void set( CompatibleFunctionPointer ptr )
	{
		...
		{
			/*
			CompatibleFunctionPointer* buffer = reinterpret_cast<CompatibleFunctionPointer*>(&m_Data);
			new(buffer) CompatibleFunctionPointer(ptr);

			m_ManagerFctPtr = nullptr;
			m_InvokeFctPtr = &invoke_func_ptr;
			*/
	...
	set( const FunctorArgT& ftor )
	{
		...
		// generate entry call
		m_InvokeFctPtr = &invoke_functor<FunctorT>;
	}
	...
	static RetT invoke_functor( ArgsT&&... args, const void* dataPtr )
	{
		FunctorT* functor = ( FunctorT* )const_cast<void*>( dataPtr );
		return ( *functor )( std::forward<ArgsT>( args )... );
	}

	static RetT invoke_func_ptr(ArgsT&&... args, const void* dataPtr)
	{
		CompatibleFunctionPointer* funcPtr = (CompatibleFunctionPointer*)const_cast<void*>(dataPtr);
		return (*funcPtr)(std::forward<ArgsT>(args)...);
	}
